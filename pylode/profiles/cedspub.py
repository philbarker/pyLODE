...
try:
    ...
    from .rdf_elements import ONTDOC, AGENT_PROPS, ONT_PROPS, CLASS_PROPS,CONCEPT_PROPS, SCHEME_PROPS, NI_PROPS, PROP_PROPS
...
#            g.add((s_, RDF.type, RDFS.Class))
        ...
        # ConceptSchemes
        # Don't want to list these twice, just as concept schemes, 
        # so remove OWL.Class typing
        for s_ in g.subjects(RDF.type, SKOS.ConceptScheme) :
            g.remove((s_, RDF.type, OWL.Class))
...
#            self.ont.subjects(predicate=RDF.type, object=PROF.Profile),
    ...
    def _make_main_sections(self):
        with self.content:
            ...
            if (None, RDF.type, SKOS.ConceptScheme) in self.ont:
                d = section_html(
                    "Controlled Vocabularies (also OWL Classes)",
                    self.ont,
                    self.back_onts,
                    self.ns,
                    SKOS.ConceptScheme,
            ...
            if (None, RDF.type, SKOS.Concept) in self.ont:
                d = section_html(
                    "Controlled Vocabulary Items (also Named Individuals)",
                    self.ont,
                    self.back_onts,
                    self.ns,
                    SKOS.Concept,
